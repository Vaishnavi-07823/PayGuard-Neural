...
WHAT: Takes the final_score and full breakdown from Stages 1+2, then
calls the Claude API (claude-sonnet-4-6) to generate a plain-language
explanation of why the UPI ID received that risk score.

WHY: Raw numerical scores are meaningless to regular users. A human-
readable explanation like "This UPI ID was linked to 4 complaints in
72 hours..." makes the system actually useful for non-technical people.

REAL COMPONENT: This is a REAL API call to Anthropic's Claude API —
not simulated. The API key is read from the ANTHROPIC_API_KEY
environment variable. If the key is missing, the endpoint still works
but returns a static fallback message instead of crashing.
"""
...
def generate_explanation(upi_id: str, final_score: float, risk_level: str, breakdown: dict) -> str:
    ...
    Args:
        upi_id: The UPI ID that was checked
        final_score: The combined final score (0-100)
        risk_level: 'SAFE', 'CAUTION', or 'HIGH_RISK'
        breakdown: Dict containing risk_engine_score, chain_score,
                   and sub-component details (db_result, ml_result,
                   chain details, etc.)
    ...
    if not api_key:
        # Graceful fallback — don't crash, just explain the situation
        return _generate_fallback_explanation(upi_id, final_score, risk_level, breakdown)

    try:
        # Import anthropic only when we actually have a key
        import anthropic

        client = anthropic.Anthropic(api_key=api_key)

        # Build a detailed prompt with all the scoring context
        ...
        # If the API call fails for any reason, fall back to static message
...
def _build_prompt(upi_id: str, final_score: float, risk_level: str, breakdown: dict) -> str:
    """
    Builds the prompt sent to Claude with all context needed to
    ...
    prompt = f"""You are a fraud risk explainer for PayGuard-Neural, a UPI payment safety system in India.
...
def _generate_fallback_explanation(upi_id: str, final_score: float, risk_level: str, breakdown: dict) -> str:
    """
    Generates a static fallback explanation when the Claude API key
    is not configured or the API call fails.

    WHY: The system should NEVER crash just because the AI explanation
    is unavailable. The score itself is the most important output.
    """
    db_result = breakdown.get("db_result", {})
    report_count = db_result.get("report_count", 0)
    chain_trace = breakdown.get("chain_trace", [])
