...
the report_count into a sub-score (0-100). A higher report_count means
...
def get_db_match_score(upi_id: str) -> dict:
    ...
    if result is None:
        # UPI ID not in our database at all — no evidence of fraud
        return {
            "db_score": 100,
            "report_count": 0,
            "found_in_db": False,
    ...
    # Convert report_count to a safety score (higher = safer)
    if report_count == 0:
        db_score = 100
    elif report_count <= 2:
        db_score = 65
        ...
        # 11+ reports — overwhelmingly flagged
        db_score = 5
